    ...
    Column(
        ...
        OutlinedTextField(
            ...
            trailingIcon = {
                IconButton(onClick = viewModel::toggleConfirmPasswordVisibility) {
                    Icon(
                        if (state.confirmPasswordVisible) Icons.Default.Visibility else Icons.Default.VisibilityOff,
                        contentDescription = null, tint = Color.Gray
                    )
                }
            },
            ...
            shape = RoundedCornerShape(4.dp),
            colors = OutlinedTextFieldDefaults.colors(
                focusedBorderColor = if (state.confirmPassword == state.password && state.confirmPassword.isNotEmpty())
                    Color.Green else if (state.confirmPassword.isNotEmpty()) Color.Red else Color.DarkGray,
                unfocusedBorderColor = if (state.confirmPassword == state.password && state.confirmPassword.isNotEmpty())
                    Color.Green else if (state.confirmPassword.isNotEmpty()) Color.Red else Color.DarkGray,
                focusedTextColor = Color.White,
                unfocusedTextColor = Color.White,
                cursorColor = Color.Green
        ...
        Button(
            ...
            enabled = state.email.isNotEmpty() && state.password.isNotEmpty() &&
                    state.username.isNotEmpty() && state.password == state.confirmPassword && !state.isLoading
        ) {
            if (state.isLoading) {
